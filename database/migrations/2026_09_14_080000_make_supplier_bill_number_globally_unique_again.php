...
return new class extends Migration {
    ...
    {
        $duplicates = DB::table('company_bills')->select('supplier_id', 'bill_no')
            ->whereNotNull('supplier_id')->groupBy('supplier_id', 'bill_no')
            ->havingRaw('COUNT(*) > 1')->get();

        foreach ($duplicates as $duplicate) {
            $ids = DB::table('company_bills')->where('supplier_id', $duplicate->supplier_id)
                ->where('bill_no', $duplicate->bill_no)->orderBy('id')->pluck('id');
