(function (global, factory) {
    ...
    ***************************************************************************** */
     ...
     */
    var yearMonthDaysMapping = Object.values(dateConfigMap).map(function (
        year
    ) {
    ...
    function findPassedDays(year, month, date) {
        try {
            ...
            var pastMonthDays =
                ...
                pastYearDays +
                monthDaysMappings[yearIndex + extraYear][extraMonth][
                    COMPLETED_DAYS
                ];
    ...
    function mapDaysToDate(daysPassed) {
        if (daysPassed < MIN_DAY || daysPassed > MAX_DAY) {
            throw new Error(
                "The epoch difference is not within the boundaries "
                    .concat(MIN_DAY, " - ")
                    .concat(MAX_DAY)
            );
        }
        ...
        var monthIndex = monthDaysMappings[yearIndex].findIndex(function (
            month
        ) {
            return (
        ...
        var date =
            monthRemainder -
            monthDaysMappings[yearIndex][monthIndex][COMPLETED_DAYS];
        return {
    ...
    function convertToAD(bsDateObject) {
        try {
            var daysPassed_2 = findPassedDays(
                bsDateObject.year,
                bsDateObject.month,
                bsDateObject.date
            );
    ...
    function convertToBS(adDateObject) {
        try {
            var daysPassed_3 = findPassedDaysAD(
    ...
    function format(bsDate, stringFormat, language) {
        return stringFormat
            .replace(
                ...
                function (match, _, matchedString) {
                    var _a;
                    switch (match) {
                        case "D":
                            return mapLanguageNumber(
                                bsDate.date.toString(),
                                language
                            );
                        case "DD":
                            return mapLanguageNumber(
                        ...
                        case "MMM":
                            return formatObj[language].month.short[
                                bsDate.month
                            ];
                        case "MMMM":
                        ...
                        case "d":
                            return mapLanguageNumber(
                                ((_a = bsDate.day) === null || _a === void 0
                                    ? void 0
                                    : _a.toString()) || "0",
                                language
                            );
                        case "dd":
                            return formatObj[language].day.short[
                                bsDate.day || 0
                            ];
                        case "ddd":
                            return formatObj[language].day.long[
                                bsDate.day || 0
                            ];
                        default:
    ...
    var NepaliDate = /** @class */ (function () {
        ...
        NepaliDate.prototype[setDayYearMonth] = function (
            year,
            month,
            date,
            day
